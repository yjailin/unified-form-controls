...
/**
 * Echo the `<main class="uf-showcase">` block — the showcase preview surface.
 *
 * Extracted from the `template_redirect` handler so both contexts can
 * render the showcase from one source:
 *   - `/?uf_showcase=1`  → frontend route wraps this in full <html><body>
 *   - `Appearance > Form controls` admin page → renders this INLINE in
 *     the admin DOM (no iframe). Avoids the WordPress.com / Calypso
 *     frontend-chrome injection problems that the iframe surfaces had
 *     (#wpadminbar duplication, #atomic-proxy-bar duplication, the
 *     performance optimizer relocating `admin-bar.min.css` into <body>,
 *     etc.). The admin document is NOT subject to those injections, so
 *     inline rendering sidesteps the entire class of bugs.
 *
 * @param array $args {
 *     Optional render flags.
 *
 *     @type bool $hide_heading Hide the outer H1 + lede paragraph. True
 *                              when the surrounding context already
 *                              provides a page title (admin page, embed
 *                              mode). Default false.
 * }
 */
function ufc_render_showcase_main( $args = array() ) {
	$args = array_merge( array( 'hide_heading' => false ), $args );
...
<body <?php body_class( 'uf-showcase-body' ); ?>>
	...
	<?php ufc_render_showcase_main( array( 'hide_heading' => ! empty( $_GET['uf_embed'] ) ) ); ?>
	<?php wp_footer(); ?>
</body>
</html>
	<?php
